...
def payment_callback(request):
    ...
    if order_status == 'PAID':
        ...
        # Send email notifications for all orders in this transaction
        orders = Order.objects.filter(transaction_id=txn_id, payment_status='Success')
        for order in orders:
            try:
                send_all_order_notifications(order)
            except Exception as e:
                # Log error but don't fail the order - emails are non-critical
                print(f"Email notification error for order {order.order_id}: {str(e)}")
